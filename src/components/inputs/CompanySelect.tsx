...
export default function CompanySelect({ id, value, onChange }: CompaniesSelectProps) {
  const defaultValue: CompanySelectOption | null = value ? { label: value.name, value: value.id, data: value } : null;
  const [optionValue, setOptionValue] = useState<CompanySelectOption | null>(defaultValue)
  const fetchData = async (query: string) => {
    try {
      const { data } = await serverFetch.get<AppUser[]>('/companies/search?q=' + query)
